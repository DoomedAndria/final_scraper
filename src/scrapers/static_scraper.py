...
class ZoomerStrategy(CategoryStrategy,BaseScraper):
    ...
    def get_brand_links(self):
        def click_see_more_button(driver: WebDriver):
            ...
            """, button)
        soup = SeleniumScraper.scrape(self.smartphones_page_url,click_see_more_button)

        brand_container = soup.select_one('div.sc-b97309d4-5')
        if not brand_container:
            raise ValueError("Brand container not found")
        brands = [span.get_text(strip=True) for span in brand_container.find_all('span') if span.get_text(strip=True)]
        links = {
...
class SiteBStrategy(CategoryStrategy,BaseScraper):
    def get_brand_links(self):
        pass
